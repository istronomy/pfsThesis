\chapter{再現実験における事象選別と結果(担当:礒部)}\label{result}
...
事象のトリガーとして図\ref{fig:na}に示す励起状態の\ce{^{22}Ne}が基底状態の\ce{^{22}Ne}になるときに放出される$1275\si{\kilo\electronvolt}$の$\gamma$線を用いた.
	...
	\begin{tabular}{cc}
		...
		\begin{minipage}[t]{0.5\hsize}
			\centering
				...
				\caption{装置の全体写真 \newline シンチレータS及び線源を取り囲む真鍮製のコリメータのみが見えており,他の構成要素はアルミニウム板の下に配されている.}
...
線源である\ce{^{22}Na}から陽電子が下方に放出された場合に,シリカエアロゲル中の電子と反応して形成されるポジトロニウムの崩壊を観測する.ポジトロニウムの崩壊によって生じる$\gamma$線をシリカエアロゲルを囲むように配置してある3つのNaI(Tl)シンチレータを用いて検出する.
ここで、コリメータをつけているのは線源からの$\gamma$線が直接シンチレータA,B,Cに入射することを防ぐためである.
図\ref{collimators}にコリメータの図を昨年度の資料~\cite{卒業発表2015}から引用し示す.
また,ポジトロニウムの生成された時刻を求めるためには,\ce{^{22}Na}の崩壊によって生じる1275keVの$\gamma$線を用いる.上方に設置してシンチレータSでこの$\gamma$線が検出された場合にポジトロニウムの生成時刻を知ることができる.
%一気に3行2列で配置する方
%\begin{figure}[H]
...
%	\begin{tabular}{cc}
%		%1枚目
%		\begin{minipage}[htbp]{0.5\hsize}
%			\centering
%			\includegraphics[width=80mm]{fig/isb/decay_t.pdf}
%			\caption{崩壊時間の分布}
%			\label{fig:dec_t_b4}
%		\end{minipage}
%		%2枚目
%		\begin{minipage}[htbp]{0.5\hsize}
%			\centering
%			\includegraphics[width=80mm]{fig/isb/cal_9075.pdf}
%			\caption{4つのシンチレータにおける較正後のエネルギースペクトル \newline 左上から時計回りにシンチレータA,B,S,C}
%			\label{fig:E-tHL_cut}
%		\end{minipage}
%	\end{tabular}
%\end{figure}
図\ref{fig:dec_t_b4}はシンチレータA,B,Cでの各崩壊時間の分布を区別せず表示したものである.ここでの崩壊時間はシンチレータSの閾値lowを超えた時刻とシンチレータA,B,Cでの閾値lowを超えた時刻の差である.
...
これは図\ref{fig:na}に示す励起状態の\ce{^{22}Ne}が基底状態になるときに放出する$1275$keVのエネルギーを持つ$\gamma$線によるものである.
...
フィッティングした結果を図\ref{fig:2_3gamma_fit}に示す.フィットの範囲は$(20,500)$である.
\begin{figure}[H]
	\centering
		\includegraphics[width=12cm]{fig/isb/2-3gamma.pdf}
		\caption{$2\gamma$領域(左)と$3\gamma$領域(右)に対するフィッティング結果}
...
また,閾値highを超えるまでの時間も$\Delta t$だけ小さくなる.%(図を入れるか考え中)%
...
第\ref{SCtrig}章で議論するように,プラスチックシンチレータを用いることで\ce{^{22}Na}の崩壊で放出される陽電子を直接トリガーとして利用できる.
